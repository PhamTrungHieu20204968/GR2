...
function CreateBlog({ accessToken, blog, edit = false }) {
  ...
  const onFinish = (values) => {
    ...
    for (let index = 0; index < tuNhayCam.length; index++) {
      const element = tuNhayCam[index];
      if (
        values?.title?.includes(element) ||
        values?.content?.includes(element)
      ) {
        message.error(
          language === 'vi'
            ? 'Nội dung bài viết có chứa từ ngữ không hợp lệ!'
            : 'この内容にはデリケートな言葉が含まれています。'
        );
        setLoading(false);
        return;
      }
